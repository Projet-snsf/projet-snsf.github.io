function toggleMenu() {document.getElementById('miniMenu').classList.toggle('show');}
function toggleSlideMenu() {document.getElementById('slideMenu').classList.toggle('show');}

function  proEditing(){
  let inputers = document.querySelectorAll('.menu-item input');
  let proEditeur = document.querySelector('.pro-editeur');
    inputers.forEach(input=>{input.disabled=false;input.classList.add('actived')})
    proEditeur.innerHTML=`Enregistrer <i class="fas fa-save"></i>`;
    proEditeur.classList.replace('btn-primary','btn-success');
}
...
function dataBuilder(geojson) {
    ...
    map = L.map('map',{
      ...
      const surveycluster = L.markerClusterGroup().addLayer(global);
...
function uppers() {
    ...
    filterForm.addEventListener('input', function() {updateData(); });
    updateData(); 
  }
uppers()
